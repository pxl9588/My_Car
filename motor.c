...
/*
 * Change the duty cycle and direction of a motor
 * @param DutyCycle (0 to 100)
 * @param left True / False
 * @param forward True / False
 */
void set_motor_duty(unsigned int dutyCycle, int left){

	// Calculate the new cutoff value
	uint16_t mod = (uint16_t) (((CLOCK/MOTOR_FREQ) * dutyCycle) / 100);

	// Set outputs
	if(left){FTM0_C1V = mod;  FTM0_C0V = 0;}
	else	{FTM0_C2V = mod;  FTM0_C3V = 0;}
}
